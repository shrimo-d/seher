...
def make_pos_difference_penalty_function(alpha):
    ...
    def penalty_function(state, control):
        uncertainty_change = jnp.mean(state.est.epistemic_std) - state.last_unc
        return alpha * jnp.maximum(uncertainty_change, 0.0)
...
def make_pos_difference_reward_function(alpha):
    ...
    def reward_function(state, control):
        uncertainty_reduction = state.last_unc - jnp.mean(
            state.est.epistemic_std
        )
        return alpha * jnp.maximum(uncertainty_reduction, 0.0)
