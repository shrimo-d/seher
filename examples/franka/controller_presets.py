"""Named controller presets and shared CLI handling for Franka experiments.

The controller implementations in :mod:`seher` stay environment agnostic.  This
module is the single source of truth for the Franka-specific controller choices
...
# Stable ARS reference used by the existing Franka experiments.
...
# Historical ARS setup used only by wobble_reward_test.py.
...
# Selected from sweep_mppi_goal_and_standard.py: strong goal-state stability
# while retaining almost all standard-run progress.
MPPI_BALANCED = replace(
    ARS_REFERENCE,
    optimizer="mppi",
    n_iter=2,
    mppi_top_k=16,
...
# Faster alternative (>10 Hz in the measured setup), with a larger quality
# trade-off in the standard rollout.
MPPI_FAST = replace(
    MPPI_BALANCED,
    n_iter=1,
    mppi_top_k=64,
...
DEFAULT_CONTROLLER_PRESET = "ars_reference"
...
def add_controller_arguments(
    ...
    *,
    ...
    ``--optimizer mppi`` to select the complete ``mppi_balanced`` preset
    instead of inheriting ARS planner defaults such as ``n_iter=10``.
    """
